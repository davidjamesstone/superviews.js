...
function superviews(rootEl, rootCtx, options) {
  ...
  var selectors = [
    withAttrName, forAttrName, textAttrName, ifAttrName,
  ...
  function processNode(el) {
    ...
    var elScopes, attrs, withAttr, forAttr,
      ifAttr, attrsAttr, textAttr, htmlAttr,
      valueAttr, bindAttr, clickAttr, forItems;
    ...
    if (valueAttr) {
      // use empty string if falsey
      var valueValue = evaluate(elScopes, valueAttr.value) || '';
      // don't update if value unchanged
      if (el.value !== valueValue) {
        el.value = valueValue;
      }
    }
